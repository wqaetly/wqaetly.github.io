  ...
  const scrollFnToDo = () => {
    ...
    if (isToc) {
      ...
      const tocItemClickFn = e => {
        ...
        e.preventDefault()
        const destId = decodeURI(target.getAttribute('href')).replace('#', '')
        const destEle = document.getElementById(destId)
        if (!destEle) return
        btf.scrollToDest(btf.getEleTop(destEle), 300)
    ...
    const findHeadPosition = top => {
      ...
      for (let i = 0; i < $articleList.length; i++) {
        ...
        // Keep this threshold aligned with scrollToDest's 70px header offset
