document.addEventListener('DOMContentLoaded', () => {
  ...
  */
  const scrollFnToDo = () => {
    let isToc = GLOBAL_CONFIG_SITE.isToc
    const isAnchor = GLOBAL_CONFIG.isAnchor
    const $article = document.getElementById('article-container')

    if (!($article && (isToc || isAnchor))) return

    const getDecodedHash = href => {
      if (!href) return ''
    ...
    const tocScrollOffset = 70
    const tocActiveTolerance = 10
    ...
    const scrollToTocHeading = (heading, immediate = false) => {
      const pos = Math.max(0, btf.getEleTop(heading) - tocScrollOffset)
      if (immediate) {
        window.scrollTo({
          top: pos,
          behavior: 'auto'
      ...
      btf.scrollToDest(btf.getEleTop(heading), 300)
    }

    let $tocLink, $cardToc, autoScrollToc, $tocPercentage, isExpand, tocHeadingMap, tocLinkMap
    let $articleList = Array.from($article.querySelectorAll('h1,h2,h3,h4,h5,h6'))
    let tocClickLock = false
    let tocClickTimer

    if (isToc) {
      const $cardTocLayout = document.getElementById('card-toc')
      $cardToc = $cardTocLayout && $cardTocLayout.querySelector('.toc-content')
      if (!$cardToc) {
        isToc = false
      } else {
        ...
        tocHeadingMap = new Map()
        tocLinkMap = new Map()

        $tocLink.forEach((link, index) => {
          let destEle = document.getElementById(getDecodedHash(link.getAttribute('href')))
          if (!(destEle && destEle.matches('h1,h2,h3,h4,h5,h6'))) {
            destEle = $articleList[index]
            if (destEle) {
              link.setAttribute('href', `#${encodeURI(createHeadingId(destEle, index))}`)
            }
          }

          if (destEle && destEle.matches('h1,h2,h3,h4,h5,h6')) {
            tocHeadingMap.set(link, destEle)
            tocLinkMap.set(destEle, link)
          }
        })

        if (tocLinkMap.size) {
          $articleList = $articleList.filter(heading => tocLinkMap.has(heading))
        }
        ...
        const tocItemClickFn = e => {
          ...
          const destEle = tocHeadingMap.get(target) || document.getElementById(getDecodedHash(target.getAttribute('href')))
          if (!destEle) return
          ...
          setTocActive(target)
          scrollToTocHeading(destEle)
          tocClickTimer = setTimeout(() => {
            if (Math.abs(destEle.getBoundingClientRect().top - tocScrollOffset) > 3) {
              scrollToTocHeading(destEle, true)
            }
        ...
        btf.addEventListenerPjax($cardToc, 'click', tocItemClickFn, true)
        ...
        autoScrollToc = item => {
          ...
          if (Math.abs(offset - middlePosition) > 1) {
    ...
    const setTocActive = currentActive => {
      if (!(isToc && currentActive)) return
    ...
    const findHeadPosition = top => {
      ...
      for (let i = 0; i < $articleList.length; i++) {
        ...
        // Keep this threshold aligned with the TOC click offset.
        // so that after a TOC click, the active highlight matches the scrolled-to heading.
        if (top + tocActiveTolerance >= btf.getEleTop(ele) - tocScrollOffset) {
          const id = ele.id
      ...
      if (detectItem === currentHeading) return

      if (isAnchor) btf.updateAnchor(currentId)

      detectItem = currentHeading

      if (isToc) {
        if (currentHeading) {
          const currentActive = tocLinkMap.get(currentHeading)
          if (!currentActive) return

          setTocActive(currentActive)
        } else {
          $cardToc.querySelectorAll('.active').forEach(i => i.classList.remove('active'))
        }
      }
    ...
    const tocScrollFn = btf.throttle(() => {
      ...
      if (!tocClickLock) findHeadPosition(currentTop)
    }, 100)
