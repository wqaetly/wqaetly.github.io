(() => {
  const btfFn = {
    ...
    scrollToDest: (pos, time = 500) => {
      ...
      // Butterfly switches classes: 'nav-fixed' (has-top-img post/page) / 'fixed' (no-top-img pages).
      // The previous implementation only checked 'fixed', which was never true on normal posts
      // and therefore skipped the 70px header offset — causing click jumps to land under the navbar.
      const isNavFixed = $header && ($header.classList.contains('nav-fixed') || $header.classList.contains('fixed'))
      if (currentPos > pos || isNavFixed) pos = pos - 70
      // Clamp lower bound to avoid jumping back to top when target is near the page top
      if (pos < 0) pos = 0
    ...
    getEleTop: ele => {
      if (!ele) return 0
      // Use getBoundingClientRect to get a reliable absolute Y, independent of
      // offsetParent chain (which breaks when ancestors use transform, fixed, or
      // contain: layout, etc.). This fixes TOC clicks jumping to the page top.
